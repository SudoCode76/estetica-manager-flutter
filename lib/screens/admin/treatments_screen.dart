...
class _TreatmentsScreenState extends State<TreatmentsScreen>
  ...
  bool _filterCategoryMissing = false;
  ...
  void initState() {
    ...
    _treatmentSearchCtrl.addListener(
  ...
  Future<void> _loadAll() async {
    ...
    try {
      final cats = await _catalogRepo.getCategorias().timeout(
        const Duration(seconds: 8),
      );
      final trats = await _catalogRepo.getTratamientos().timeout(
        const Duration(seconds: 8),
      );
      _categoriasAll = cats;
      ...
      final msg = e is TimeoutException
          ? 'Timeout al cargar catálogos (verifica conexión)'
          : e.toString();
      if (mounted)
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Error al cargar datos: $msg')));
    } finally {
      if (mounted) setState(() => _loading = false);
    }
  }

  bool _isCategoriaActiva(Map<String, dynamic> categoria) {
    return categoria['estadoCategoria'] == true ||
        categoria['estadoCategoria'] == null;
  }

  void _ensureValidTreatmentFilter() {
    final filterId = _treatmentCategoryFilter;
    final bool wasMissing = _filterCategoryMissing;
    if (filterId == null) {
      if (wasMissing) _filterCategoryMissing = false;
      return;
    }

    final bool exists = _categoriasAll.any(
      (c) => _isCategoriaActiva(c) && c['id'] == filterId,
    );

    if (!exists) {
      _treatmentCategoryFilter = null;
      _filterCategoryMissing = true;
    } else if (wasMissing) {
      _filterCategoryMissing = false;
    }
  }
  ...
  void _applyFilters() {
    if (_showDisabled) {
      ...
      _categorias = _categoriasAll.where((c) => _isCategoriaActiva(c)).toList();
      _tratamientos = _tratamientosAll
          .where(
            (t) =>
                t['estadoTratamiento'] == true ||
                t['estadoTratamiento'] == null,
          )
          .toList();
    }
    _ensureValidTreatmentFilter();
  }
  ...
  void _applyCategorySearch(String query) {
    ...
    setState(() {
      if (q.isEmpty) {
        ...
        final base = _showDisabled
            ? _categoriasAll
            : _categoriasAll.where((c) => _isCategoriaActiva(c));
        _categorias = base
            .where(
              (c) => (c['nombreCategoria']?.toString().toLowerCase() ?? '')
                  .contains(q),
            )
            .toList();
      }
  ...
  void _applyTreatmentSearch(String query) {
    ...
    setState(() {
      _ensureValidTreatmentFilter();
      List<dynamic> base = _showDisabled
          ? List<dynamic>.from(_tratamientosAll)
          : _filterTratamientosList(_tratamientosAll);
  ...
  List<dynamic> _filterTratamientosList(List<dynamic> list) {
    ...
    final activeCatIds = _categoriasAll
        .where((c) => _isCategoriaActiva(c))
        .map((c) => c['id'])
        .toSet();
    return list.where((t) {
      final bool tratActivo =
  ...
  String _getCategoriaNombreFromTratamiento(Map<String, dynamic> t) {
    ...
    final found = _categoriasAll.firstWhere(
      (c) => c['id'] == cat,
      orElse: () => null,
    );
    return found != null
        ? (found['nombreCategoria'] ?? 'Sin categoría')
        : 'Sin categoría';
  }
            ...
            onPressed: () async {
              ...
              try {
                final existing = _categoriasAll.firstWhere(
                  (c) =>
                      (c['nombreCategoria']?.toString().trim().toLowerCase() ??
                          '') ==
                      nombre.toLowerCase(),
                  orElse: () => null,
                );
                if (existing != null) {
                  if ((existing['estadoCategoria'] == true)) {
                    ...
                    // reactivar
                    await _catalogRepo.updateCategoria(
                  ...
                  await _catalogRepo.crearCategoria({
  ...
  Future<void> _showCreateTratamientoDialog() async {
    ...
    int? selectedCatId = _categorias.isNotEmpty
        ? _categorias.first['id']
        : null;
    ...
    await showDialog<bool>(
      ...
      builder: (ctx) {
        ...
        final maxW = isNarrowDialog
            ? MediaQuery.of(ctx).size.width - 32
            : 520.0;
        return AlertDialog(
          insetPadding: EdgeInsets.symmetric(
            horizontal: isNarrowDialog ? 16 : 48,
            vertical: 24,
          ),
                  ...
                  final existing = _tratamientosAll.firstWhere((t) {
                    final tname =
                        (t['nombreTratamiento']
                            ...
                            .toLowerCase()) ??
                        '';
                    final tcat = (t['categoria_tratamiento'] is Map)
                        ? t['categoria_tratamiento']['id']
                        : t['categoria_tratamiento'];
                    return tname == nombre.toLowerCase() &&
                        (selectedCatId == null
                            ? true
                            : (tcat == selectedCatId));
                  }, orElse: () => null);
                  if (existing != null) {
                    if (existing['estadoTratamiento'] == true) {
                      if (mounted)
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('El tratamiento ya existe'),
                          ),
                        );
                    } else {
                      await _catalogRepo.updateTratamiento(
                        existing['documentId'] ?? existing['id'].toString(),
                    ...
                    if (selectedCatId != null)
                      payload['categoria_tratamiento'] = selectedCatId;
  ...
  Future<void> _showEditTratamientoDialog(Map<String, dynamic> t) async {
    final nombreCtrl = TextEditingController(
      text: t['nombreTratamiento'] ?? '',
    );
    final precioCtrl = TextEditingController(
      text: t['precio']?.toString() ?? '',
    );
    int? selectedCatId = (t['categoria_tratamiento'] is Map)
        ? t['categoria_tratamiento']['id']
        : t['categoria_tratamiento'];
              ...
              onPressed: () async {
                ...
                try {
                  final Map<String, dynamic> payload = {
                    'nombreTratamiento': nombre,
                    'precio': precio,
                    'categoria_tratamiento': selectedCatId,
                    'estadoTratamiento': t['estadoTratamiento'] ?? true,
                  };
                  await _catalogRepo.updateTratamiento(
                    t['documentId'] ?? t['id'].toString(),
                    payload,
                  );
                  await _loadAll();
                } catch (e) {
  ...
  Future<void> _toggleTratamientoEstado(Map<String, dynamic> t) async {
    ...
    try {
      ...
      await _catalogRepo.updateTratamiento(docId, {
        'estadoTratamiento': newEstado,
      });
      await _loadAll();
    } catch (e) {
      if (mounted)
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              ...
              try {
                await _catalogRepo
                    .updateCategoria(c['documentId'] ?? c['id'].toString(), {
                      'nombreCategoria': nombre,
  ...
  Widget _buildCategoryListItem(
    ...
    final bool activo =
        c['estadoCategoria'] == true || c['estadoCategoria'] == null;
      ...
      child: ListTile(
        ...
        trailing: Row(
          ...
          children: [
            ...
            IconButton(
              icon: Icon(
                activo ? Icons.visibility_off_outlined : Icons.restore_outlined,
                size: 18,
              ),
  ...
  Widget _buildCategoriesTab(ColorScheme cs, TextTheme tt, bool isNarrow) {
    return Column(
      ...
      children: [
        ...
        if (_showDisabled)
          Padding(
            padding: EdgeInsets.symmetric(horizontal: isNarrow ? 0 : 8.0),
            child: Align(
              alignment: Alignment.centerLeft,
              child: Text(
                'Incluyendo categorías desactivadas',
          ...
          child: _categorias.isEmpty
              ? Center(
                  child: Padding(
                    ...
                    child: Text('No hay categorías', style: tt.bodyMedium),
                  ),
                )
              : ListView.builder(
  ...
  Widget _buildTratamientoItem(
    Map<String, dynamic> t,
    ColorScheme cs,
    TextTheme tt,
    bool isNarrow,
  ) {
    final bool activoTrat =
        t['estadoTratamiento'] == true || t['estadoTratamiento'] == null;
          ...
          children: [
            ...
            if (!activoTrat)
              Padding(
                padding: const EdgeInsets.only(top: 2),
                child: Text(
                  'No disponible hasta reactivarlo',
                  style: tt.bodySmall?.copyWith(
                    color: cs.onSurfaceVariant.withValues(alpha: 0.7),
                    fontStyle: FontStyle.italic,
                  ),
  ...
  Widget _buildTreatmentsTab(ColorScheme cs, TextTheme tt, bool isNarrow) {
    final List<dynamic> activeCategories = _categoriasAll
        .where((c) => _isCategoriaActiva(c))
        .toList();
    final bool filterExists =
        _treatmentCategoryFilter != null &&
        activeCategories.any((c) => c['id'] == _treatmentCategoryFilter);
    final int? dropdownValue = filterExists ? _treatmentCategoryFilter : null;
        ...
        Padding(
          ...
          child: Column(
            children: [
                  ...
                  Container(
                    ...
                    child: IconButton(
                      ...
                      constraints: BoxConstraints(
                        minWidth: isNarrow ? 40 : 48,
                        minHeight: isNarrow ? 40 : 48,
                      ),
                      tooltip: _showDisabled
              ...
              if (_showDisabled)
                Align(
                  ...
                  child: Padding(
                    ...
                    child: Text(
                      'Incluyendo tratamientos desactivados',
                      style: tt.bodySmall?.copyWith(color: cs.onSurfaceVariant),
                    ),
                  ),
                ),
              ...
              Container(
                ...
                child: DropdownButtonFormField<int?>(
                  ...
                  items: [
                    ...
                    ...activeCategories.map<DropdownMenuItem<int?>>(
                      (c) => DropdownMenuItem<int?>(
                        value: c['id'],
                        child: Text(
                          c['nombreCategoria'] ?? '-',
              ...
              if (_filterCategoryMissing)
                Padding(
                  padding: const EdgeInsets.only(top: 8, left: 8, right: 8),
                  child: Text(
                    'La categoría seleccionada ya no está disponible. Mostrando todas.',
          ...
          child: Card(
            child: _tratamientos.isEmpty
                ? Center(
                    child: Padding(
                      padding: const EdgeInsets.all(24.0),
                      child: Text('No hay tratamientos', style: tt.bodyMedium),
                    ),
                  )
                : ListView.builder(
                    padding: const EdgeInsets.fromLTRB(
                      ...
                      0,
                      140,
                    ), // espacio extra abajo para FAB
                    itemCount: _tratamientos.length,
                    itemBuilder: (context, index) => _buildTratamientoItem(
                      _tratamientos[index] as Map<String, dynamic>,
                      cs,
                      tt,
                      isNarrow,
                    ),
                  ...
                  child: TabBarView(
                    ...
                    children: [
                      _loading
                          ...
                          : _buildCategoriesTab(cs, tt, isNarrow),
                      _loading
                          ? const Center(child: CircularProgressIndicator())
                          : _buildTreatmentsTab(cs, tt, isNarrow),
                    ],
                  ),
                ),
              ),
