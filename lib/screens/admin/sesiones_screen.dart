...
class _SesionesScreenState extends State<SesionesScreen>
  ...
  void _onSearchChanged(String value) {
    ...
    _debounceSearch = Timer(const Duration(milliseconds: 400), () async {
      ...
      try {
        debugPrint(
          'Sesiones: buscando "${_search.trim()}" en sucursal $sucursalId',
        );
        final res = await Provider.of<TicketProvider>(context, listen: false)
            .searchSessions(
              query: _search.trim(),
              sucursalId: sucursalId,
              estadoSesion: _filtroEstado,
    ...
    try {
      ...
      String _extractNombreCliente(dynamic item) {
        try {
          ...
          final Map<String, dynamic> j = item is Map
      ...
      for (final item in agenda) {
        try {
          ...
          try {
            final possible = (item is Map && item['cliente'] is String)
                ? item['cliente']
                : '';
            if (possible != null &&
                _normalize(possible.toString()).contains(term)) {
              filtered.add(item);
              continue;
            }
          } catch (_) {}
      ...
      // debug
      debugPrint(
        'Sesiones: filtro="$term" -> ${filtered.length}/${agenda.length} coincidencias',
      );
      return filtered;
    } catch (e) {
      return agenda;
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'No se encontraron sesiones para "${_search.trim()}"',
                              style: theme.textTheme.titleMedium?.copyWith(
                                color: colorScheme.onSurfaceVariant,
                      ...
                      child: RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            final sesion = AgendaItem.fromJson(
                              displayed[index],
                            );
                            return _SesionCard(
                              ...
                              onMarcarAtendida: () async {
                                final confirmed = await showDialog<bool>(
                                  ...
                                  builder: (ctx) => AlertDialog(
                                    ...
                                    content: Text(
                                      '¿Confirmar que la sesión de ${sesion.nombreCliente} fue atendida?',
                                    ),
                                    actions: [
                                      TextButton(
                                        onPressed: () =>
              ...
              Row(
                children: [
                  CircleAvatar(
                    ...
                    child: Text(
                      sesion.nombreCliente.isNotEmpty
                          ? sesion.nombreCliente[0].toUpperCase()
                          : '?',
                      style: theme.textTheme.titleMedium?.copyWith(
                        color: colorScheme.onPrimaryContainer,
                        fontWeight: FontWeight.bold,
