...
class ReportChart extends StatelessWidget {
  ...
  data; // items: { 'date': 'YYYY-MM-DD' or 'YYYY-MM', 'payments': num, 'pendingDebt': num }
  final bool isBar;
  ...
  Widget build(BuildContext context) {
    ...
    final combined = <double>[]
      ..addAll(payments)
      ..addAll(debts);
    final maxY = (combined.isEmpty
        ? 1.0
        : (combined.reduce((a, b) => a > b ? a : b) * 1.2));
    ...
    return SizedBox(
      ...
      child: Card(
        ...
        child: Padding(
          ...
          child: isBar
              ? _buildBarChart(context, payments, debts, maxY, xLabels)
              : _buildLineChart(context, payments, maxY, xLabels),
        ),
      ),
    );
  }
  ...
  Widget _buildLineChart(
    ...
    final spots = List<FlSpot>.generate(
      payments.length,
      (i) => FlSpot(i.toDouble(), payments[i]),
    );
  ...
  Widget _buildBarChart(
    ...
    final count = payments.length < debts.length
        ? payments.length
        : debts.length;
