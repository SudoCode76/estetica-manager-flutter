...
class CatalogRepository {
      ...
      return (data as List<dynamic>).map((e) {
        if (e is Map<String, dynamic>) {
          return {
            ...
            'nombreCategoria':
                e['nombreCategoria'] ?? e['nombre_categoria'] ?? e['name'],
            // Leemos estadocategoria (DB) y lo guardamos como estadoCategoria (App)
            'estadoCategoria':
                e['estadoCategoria'] ?? e['estadocategoria'] ?? true,
      ...
      return (data as List<dynamic>).map((e) {
        if (e is Map<String, dynamic>) {
          return {
            ...
            'nombreTratamiento':
                e['nombretratamiento'] ?? e['nombreTratamiento'] ?? e['name'],
            'precio': e['precio'],
            // Leemos estadotratamiento (DB) y lo guardamos como estadoTratamiento (App)
            'estadoTratamiento':
                e['estadotratamiento'] ?? e['estadoTratamiento'] ?? true,
            'categoria_tratamiento':
                e['categoria_id'] ?? e['categoria_tratamiento'] ?? null,
            'created_at': e['created_at'],
          };
        }
        return e;
      ...
      return (data as List<dynamic>).map((e) {
        if (e is Map<String, dynamic>) {
          return {
            'id': e['id'],
            'nombreSucursal':
                e['nombreSucursal'] ?? e['nombresucursal'] ?? e['name'],
          };
        }
        return e;
      }).toList();
  ...
  Future<Map<String, dynamic>> crearCategoria(
    ...
    try {
      ...
      // Mapeo App -> DB
      if (categoria.containsKey('nombreCategoria'))
        payload['nombreCategoria'] = categoria['nombreCategoria'];
      // Nota: Postgres suele ser case-insensitive para columnas sin comillas, pero mejor usar minúsculas si falla
      if (categoria.containsKey('estadoCategoria'))
        payload['estadoCategoria'] = categoria['estadoCategoria'];

      final res = await _client
          .from('categoriaTratamiento')
          .insert(payload)
          .select()
          .single();
  ...
  Future<Map<String, dynamic>> updateCategoria(
    ...
    try {
      ...
      if (idInt == null)
        throw Exception('ID de categoría inválido: $documentId');

      // CORRECCIÓN: Mapear las llaves antes de enviar
      final payload = <String, dynamic>{};
      if (changes.containsKey('nombreCategoria'))
        payload['nombreCategoria'] = changes['nombreCategoria'];
  ...
  Future<Map<String, dynamic>> crearTratamiento(
    ...
    try {
      ...
      if (tratamiento.containsKey('nombreTratamiento'))
        payload['nombretratamiento'] = tratamiento['nombreTratamiento'];
      if (tratamiento.containsKey('precio'))
        payload['precio'] = tratamiento['precio'];
      if (tratamiento.containsKey('estadoTratamiento'))
        payload['estadotratamiento'] = tratamiento['estadoTratamiento'];
  ...
  Future<Map<String, dynamic>> updateTratamiento(
    ...
    try {
      final idInt = int.tryParse(documentId.toString());
      if (idInt == null)
        throw Exception('ID de tratamiento inválido: $documentId');

      // CORRECCIÓN IMPORTANTE: Mapear 'estadoTratamiento' -> 'estadotratamiento'
      final payload = <String, dynamic>{};

      if (changes.containsKey('nombreTratamiento'))
        payload['nombretratamiento'] = changes['nombreTratamiento'];
      if (changes.containsKey('precio')) payload['precio'] = changes['precio'];

      // Aquí arreglamos el error de "Column not found":
      if (changes.containsKey('estadoTratamiento'))
        payload['estadotratamiento'] = changes['estadoTratamiento'];
